...
{
    m_linearThresholdDistance  = linear;      /* 线性移动阈值 */
    m_angularThresholdDistance = angular;    /* 角度移动阈值 */
    m_resampleThreshold        = resampleThreshold; /* 重采样阈值   */
...
/* 初始化 */
/* size 是粒子的数目 */
/* double xmin, double ymin,double xmax, double ymax, 地图的边界 */
/* double delta 地图分辨率 */
/* OrientedPoint initialPose 机器人的初始朝向 */
void GridSlamProcessor::init(unsigned int size, double xmin, double ymin,double xmax, double ymax, double delta, OrientedPoint initialPose) 
{
	...
	{
        m_particles.push_back(Particle(lmap));  /* 每个粒子有独立的地图数据 */
...
bool GridSlamProcessor::processScan(const RangeReading & reading,int adaptParticles) 
{
    ...
    m_linearDistance  += sqrt(move * move);   /* 累加机器人移动的距离 */
    m_angularDistance += fabs(move.theta);    /* 累加机器人转动的角度 */

    // if the robot jumps throw a warning
	/* 大于一定的角度，则输出信息, 可以参看源代码中的注释 */
	/* m_distanceThresholdCheck 的缺省值是 20             */
    if (m_linearDistance > m_distanceThresholdCheck)
	{
        cerr   << "***********************************************************************"<< endl;
		cerr   << "********** m_linearDistance > m_distanceThresholdCheck   *************" << endl;
    }
	...
	if (!m_count || m_linearDistance >= m_linearThresholdDistance || m_angularDistance >= m_angularThresholdDistance || (period_ >= 0.0 && (reading.getTime() - last_update_time_) > period_)) 
	{
        ...
        cerr << "Reading Pose= " << reading.getPose().x << " " << reading.getPose().y << " " << reading.getPose().theta << endl;
        ...
        if (m_count > 0) 
		{
			...
			printf("scanMatch time = %f\n", duration);


            // onScanmatchUpdate(); /* 空函数 */
			...
			/* 进行重采样 */
			/* 在重采样的内部完成了 computeActiveArea 和 registerScan 的计算  */
